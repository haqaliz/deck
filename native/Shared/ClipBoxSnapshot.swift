...
enum HostClipBoardSampler {
    ...
    static func snapshot(maxCount: Int) -> ClipBoxSnapshot? {
        ...
        // A password manager marks its copies concealed (and often transient)
        // precisely so that history tools skip them. Reading the string anyway
        // would write the user's password into clipbox.json and render it on
        // their desktop for the next 20 copies. Keep the history untouched and
        // only refresh the timestamp, exactly as for an unreadable pasteboard.
        if ClipClassifier.isExcluded(types: typeIdentifiers) {
...
enum ClipClassifier {
    ...
    /// Pasteboard markers that mean "do not record this copy".
    ///
    /// The `org.nspasteboard.*` names are the de-facto convention from
    /// nspasteboard.org, honoured by 1Password, Bitwarden, KeePassXC, Keychain
    /// Access and the rest: *Concealed* is a secret, *Transient* is meant to
    /// live for seconds, *AutoGenerated* was not a deliberate user copy.
    /// `com.agilebits.onepassword` is 1Password's own older marker, still set
    /// by some versions that predate the convention.
    ...
    /// True when the pasteboard carries any marker asking history tools to
    /// ignore it. Checked before the kind, because an excluded pasteboard is
    /// dropped whatever it would have classified as.
    static func isExcluded(types: [String]) -> Bool {
        !Set(types).isDisjoint(with: excludedTypes)
    }
