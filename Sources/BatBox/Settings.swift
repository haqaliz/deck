...
struct BatBoxSettings: Codable {
    var showChart = true
    var showStatus = true
    var levelColor = CodableColor(.green)
    var launchAtLogin = false
}

@MainActor
final class SettingsStore: ObservableObject {
    @Published var settings: BatBoxSettings {
        didSet { save() }
    }
    ...
    private static var fileURL: URL {
        ...
        return base.appendingPathComponent("BatBox/settings.json")
    }

    init() {
        if
            let data = try? Data(contentsOf: Self.fileURL),
            let decoded = try? JSONDecoder().decode(BatBoxSettings.self, from: data)
        {
            settings = decoded
        } else {
    ...
    private static var launchAgentURL: URL {
        ...
        return dir.appendingPathComponent("LaunchAgents/com.batbox.widget.plist")
    }
