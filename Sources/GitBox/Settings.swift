...
struct GitBoxSettings: Codable, Equatable {
    ...
    var showRepos = true
    var repoCount = 5
    var barColor = CodableColor(.blue)
    var todayColor = CodableColor(.orange)
    var scanDepth = 4
...
final class SettingsStore: ObservableObject {
    @Published var settings: GitBoxSettings {
        didSet { save() }
    }
    ...
    private static var fileURL: URL {
        ...
        return base.appendingPathComponent("GitBox/settings.json")
    }

    init() {
        if
            let data = try? Data(contentsOf: Self.fileURL),
            let decoded = try? JSONDecoder().decode(GitBoxSettings.self, from: data)
        {
            settings = decoded
        } else {
    ...
    private static var launchAgentURL: URL {
        ...
        return dir.appendingPathComponent("LaunchAgents/com.gitbox.widget.plist")
    }
